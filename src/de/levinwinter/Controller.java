    ...
    public void goClicked() throws IOException {
        ...
        for(File courseFile : getSubdirs(path)) {
            ...
            for(File studentFile : getFiles(courseFile.toPath())) {
                course.addStudent(new Student(studentFile.getName().substring(0, studentFile.getName().lastIndexOf('.'))));
            }
        }

        for(File course : getSubdirs(path)) {
            pptx.createSlide(sectionLAY).getPlaceholder(0).setText(course.getName());
            for(File student : getFiles(course.toPath())) {
                XSLFSlide slide = pptx.createSlide(studentLAY);
                slide.getPlaceholder(1).setText(student.getName().substring(0, student.getName().lastIndexOf('.')));
                byte[] picture = IOUtils.toByteArray(new FileInputStream(student));
                PictureData pictureData = pptx.addPicture(picture, PictureData.PictureType.JPEG);
                XSLFPictureShape pictureINslide = slide.createPicture(pictureData);
                pictureINslide.setAnchor(slide.getPlaceholder(0).getAnchor());
                slide.removeShape(slide.getPlaceholder(0));
                i++;
                System.out.println((float) i/totalStudents);
                //progressIndicator.setProgress(round(i/totalStudents*100));
            }
        }
